...
export default function GridScan({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && typeof orientationAPI.requestPermission === 'function') {
        try {
  ...
  useEffect(() => {
    ...
    if (chromaRef.current) {
      (chromaRef.current as any).offset.set(chromaticAberration, chromaticAberration);
    }
  }, [
    lineThickness,
    linesColor,
...
function smoothDampVec2(
  ...
  out.copy(t.clone().add(change.add(temp).multiplyScalar(exp)));
